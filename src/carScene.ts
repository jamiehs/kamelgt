...
const A90_SPRITES = Array.from(
    { length: 27 },
    (_, i) => `${SPRITE_DIR}/a90${String(i + 1).padStart(2, '0')}.png`,
);

const ZXT_SPRITES = Array.from(
    { length: 14 },
    (_, i) => `${SPRITE_DIR}/zxt${String(i + 1).padStart(2, '0')}.png`,
);
...
function marginX(side: Side): number {
    const resolved = side === 'random' ? (Math.random() < 0.5 ? 'left' : 'right') : side;
    ...
    return resolved === 'left' ? window.innerWidth * 0.06 : window.innerWidth * 0.94;
}

function waypointForSection(el: Element): { x: number; y: number } {
    const id = el.id;
    const config = SECTION_CONFIGS[id] ?? {
        side: 'random' as Side,
        anchor: 'center' as VerticalAnchor,
    };
...
export function initCarScene(): () => void {
    ...
    const allRadii = shuffle(
        Array.from({ length: totalCount }, (_, i) => ARRIVAL_BASE + i * ARRIVAL_SPACING),
    );
