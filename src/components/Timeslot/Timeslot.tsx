...
const wrapEmojiAtStart = (input: string) => {
    let pattern = new RegExp(/^(\p{Emoji})/gu);
    let found = input.match(pattern);
    let inputWithoutFirstChar = input.replace(pattern, '');
    if (found && found.length) {
        return [
            <span key={input} className="emoji">
                {found}
            </span>,
            inputWithoutFirstChar,
        ];
    }
    return input;
};
            ...
            <div className="Timeslot" data-highlight={highlight}>
                ...
                {regularity && (
                    <div className="official-likelihood">
                        <div className="badge">{regularity}</div>
                    </div>
                )}
                <div className="timeslot-date">
                    <div className="date-gmt">
                        ...
                        <div className="date-time">
                            {dayLabels[dayIndex]} {time}
                        </div>
                    </div>
                    <div className="date-local">
                        <div className="date-label">{tz.replace('_', ' ')}</div>
                        <div className="date-time">
                            {nextRaceDayLocal} {stripLeadingZeroIfAmericas(nextRaceTimeLocal, tz)}
                        </div>
                    </div>
                </div>
                    ...
                    {gtoSof && gtpSof ? (
                        <div>
                            <div className="badge sof">
                                <abbr title="Strength of Field">SoF</abbr>:{' '}
                                <span
                                    className="sof-combined"
                                    title={`GTP: ${gtpSof}, GTO: ${gtoSof}`}
                                >
                                    {numberAsK(avgSof)}
                                </span>
                            </div>
                ...
                <div className="timeslot-info">{children}</div>
            </div>
        );
    }
}
