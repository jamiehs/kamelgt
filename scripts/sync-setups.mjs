...
function printPreview(car, track, pairs, existing, suggested) {
    ...
    const pruneNote =
        total > MAX_SETUPS
            ? `  ⚠  ${total} race setups after additions (max ${MAX_SETUPS} suggested)`
            : '';
    console.log(`\n${car} / ${track}${pruneNote}`);
...
async function handleNewTrack(car, track, pairs, content) {
    ...
    const existing = await prompt(
        '  Enter existing export name to link (or press enter to create new): ',
    );
...
if (targetTrack) {
    ...
    if (newSetups.length === 0) {
        console.log(
            'No new setup files found. (All .sto files in public/setups/ are already tracked by git.)',
        );
        process.exit(0);
    }
...
for (const [key, rawSetups] of groups) {
    ...
    if (!resolvedExport) {
        ...
        if (prefixMatch) {
            ...
            const expOverride = await prompt(
                `  Export name (press enter to accept "${prefixMatch}"): `,
            );
            ...
            if (folderOverride.trim() && folderOverride.trim() !== track) {
                ...
                for (const c of CARS) {
                    ...
                    try {
                        ...
                        console.log(`  → Renamed ${c}/${track} → ${resolvedTrack}`);
                    } catch {
                        /* dir may not exist for this car */
                    }
                }
            }
        } else {
    ...
    const setupsWithMeta = setups.map((s) => {
        const meta = readMeta(
            path.join(PROJECT_ROOT, 'public/setups', car, resolvedTrack, s.filename),
        );
    ...
    try {
        ...
        if (!hasSetupsBlock(currentContent, resolvedExport)) {
            ...
            currentContent = addSetupsToExport(
                currentContent,
                resolvedExport,
                resolvedTrack,
                entriesMap,
            ...
            console.log(
                `  ⚠  Could not insert into "${car}" array in ${resolvedExport} — arrays may be commented out. Add manually.`,
            );
        }
    }
}
