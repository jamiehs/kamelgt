...
describe('insertEntries', () => {
  it('inserts a new entry into an existing car array', () => {
    const result = insertEntries(SAMPLE, 'SUMMIT_POINT', 'audi90gto', [formatEntry('summit-point', 'new_R.sto', false)]);
  ...
  it('inserts into empty nissangtpzxt array', () => {
    const result = insertEntries(SAMPLE, 'SUMMIT_POINT', 'nissangtpzxt', [formatEntry('summit-point', 'nissan_R.sto', false)]);
...
describe('appendNewExport', () => {
  ...
  it('preserves original content', () => {
    const result = appendNewExport(SAMPLE, 'NEW_TRACK', 'New Track', 'new-track', { audi90gto: [], nissangtpzxt: [] });
    expect(result).toContain('existing_R.sto');
  });
});
