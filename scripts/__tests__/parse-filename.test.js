...
describe('detectType', () => {
    it('detects qual from exact Q token', () => {
        expect(detectType('A90_Fuji_Y_Gijsen_Q.sto')).toMatchObject({
            type: 'qual',
            ambiguous: false,
        });
    });
    it('detects qual from versioned q1 token', () => {
        expect(detectType('maf_summit_25s2_q1.sto')).toMatchObject({
            type: 'qual',
            ambiguous: false,
        });
    });
    it('detects qual from versioned Q82 token', () => {
        expect(detectType('Lgo26S2_Adelaide_Q82.sto')).toMatchObject({
            type: 'qual',
            ambiguous: false,
        });
    });
    it('detects qual from "quali" keyword', () => {
        expect(detectType('fuji-22-S3-quali-01.sto')).toMatchObject({
            type: 'qual',
            ambiguous: false,
        });
    });
    it('detects qual from "qualifying" keyword', () => {
        expect(detectType('SummitPoint_qualifying.sto')).toMatchObject({
            type: 'qual',
            ambiguous: false,
        });
    });
    it('detects race from exact R token', () => {
        expect(detectType('A90_Fuji_Y_Gijsen_R.sto')).toMatchObject({
            type: 'race',
            ambiguous: false,
        });
    });
    it('detects race from versioned r1 token', () => {
        expect(detectType('maf_summit_25s2_r1.sto')).toMatchObject({
            type: 'race',
            ambiguous: false,
        });
    });
    ...
    it('detects race from "race" keyword', () => {
        expect(detectType('Fuji_S3_2022_Race.sto')).toMatchObject({
            type: 'race',
            ambiguous: false,
        });
    });
    ...
    it('detects race from versioned R80 token', () => {
        expect(detectType('Lgo26S2_Adelaide_R82.sto')).toMatchObject({
            type: 'race',
            ambiguous: false,
        });
    });
